...
def elGamalEliptico(p, a, b, G, da, db, m):

    f = open("euclidesExtendido.txt", "w")
    ...
    clavePublicaBob = sumaPuntos(a, db, G, p, f)
    clavePublicaAlice = sumaPuntos(a, da, G, p, f)
    ...
    claveSecretaCompartidaAlice = sumaPuntos(a, da, clavePublicaBob, p, f)
    claveSecretaCompartidaBob = sumaPuntos(a, db, clavePublicaAlice, p, f)
    ...
    # Ciframos el mensaje:
    mensajeCifrado = sumaPuntosDistintos(p, claveSecretaCompartidaAlice, mensajeCodificado, f)
